...
def setup_snapshot_image_grid(G, training_set,
    ...
    for idx in range(gw * gh):
        x = idx % gw; y = idx // gw
        while True:
            real, label, probimage, wellface = training_set.get_minibatch_np(1) # 
...
def train_progressive_gan(
    ...
    with tf.name_scope('Inputs'):
        ...
        reals, labels, probimages, wellfacies  = training_set.get_minibatch_tf(minibatch_in)
        ...
        probimages_split    = tf.split(probimages, config.num_gpus)
    ...
    for gpu in range(config.num_gpus):
        with tf.name_scope('GPU%d' % gpu), tf.device('/gpu:%d' % gpu):
            ...
            probimages_gpu = probimages_split[gpu]
            with tf.name_scope('G_loss'), tf.control_dependencies(lod_assign_ops):
                G_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, lod = lod_in, labels = labels_gpu, well_facies = wellfacies_gpu, prob_images = probimages_gpu, minibatch_size=minibatch_split, **config.G_loss)
            with tf.name_scope('D_loss'), tf.control_dependencies(lod_assign_ops):
                D_loss = tfutil.call_func_by_name(G=G_gpu, D=D_gpu, opt=D_opt, minibatch_size=minibatch_split, reals=reals_gpu, labels=labels_gpu, well_facies = wellfacies_gpu, prob_images = probimages_gpu, **config.D_loss)
    ...
    grid_size, grid_reals, grid_labels, grid_wellfacies, grid_latents, grid_probimages = setup_snapshot_image_grid(G, training_set, **config.grid)  
    sched = TrainingSchedule(total_kimg * 1000, training_set, **config.sched)    
    grid_wellfacies_process = np.concatenate(((grid_wellfacies > 0),  (grid_wellfacies - 1) * (grid_wellfacies > 0)), 1)    
    grid_fakes = Gs.run(grid_latents, grid_labels, grid_wellfacies_process, grid_probimages, minibatch_size=sched.minibatch//config.num_gpus)
    ...
    misc.save_image_grid(grid_probimages, os.path.join(result_subdir, 'probimages.png'), drange=[0, 1], grid_size=grid_size)    
