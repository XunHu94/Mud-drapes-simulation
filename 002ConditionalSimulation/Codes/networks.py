...
def G_paper(
    ...
    probimages_in,                      # Forth input: probimages [minibatch, 1, resolution, resolution].
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    # Building blocks.
    def block(x, prob, wellfc, res): # res = 2..resolution_log2
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if res == 2: # 4x4
                ...
                with tf.variable_scope('Add_Prob'):
                    prob_downscaled = downscale2d(prob, factor=int(resolution/(2**res)))
    ...
    if structure == 'linear':
        x = block(combo_in, probimages_in, wellfacies_in, 2)
        images_out = torgb(x, 2)
        for res in range(3, resolution_log2 + 1):
            lod = resolution_log2 - res
            x = block(x, probimages_in, wellfacies_in, res)
            img = torgb(x, res)
    ...
    if structure == 'recursive':
        def grow(x, prob, wellfc, res, lod):
            ...
            if lod > 0: img = cset(img, (lod_in < lod), lambda: grow(y, prob, wellfc, res + 1, lod - 1))
            return img()
        images_out = grow(combo_in, probimages_in, wellfacies_in, 2, resolution_log2 - 2)
